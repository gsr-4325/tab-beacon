...
async function startAnimation(){stopAnimation();if(!state.baseIconDataUrl)state.baseIconDataUrl=await resolveBaseIconDataUrl(state.originalIcons);state.animationFrames=await generateSpinnerFrames(state.baseIconDataUrl);state.animationFrameIndex=0;setGeneratedIcon(state.animationFrames[0]);state.animationTimer=window.setInterval(()=>{if(!state.animationFrames?.length)return;state.animationFrameIndex=(state.animationFrameIndex+1)%state.animationFrames.length;setGeneratedIcon(state.animationFrames[state.animationFrameIndex]);},FRAME_INTERVAL_MS);}
function stopAnimation(){if(state.animationTimer){clearInterval(state.animationTimer);state.animationTimer=null;}}
async function generateSpinnerFrames(baseIconDataUrl){const baseImage=await loadImage(baseIconDataUrl);const frames=[];for(let frameIndex=0;frameIndex<FRAME_COUNT;frameIndex+=1){const canvas=document.createElement("canvas");canvas.width=32;canvas.height=32;const ctx=canvas.getContext("2d");ctx.drawImage(baseImage,0,0,32,32);ctx.fillStyle="rgba(7, 12, 24, 0.42)";ctx.beginPath();ctx.roundRect(0,0,32,32,8);ctx.fill();ctx.beginPath();ctx.fillStyle="rgba(37, 99, 235, 0.16)";ctx.arc(16,16,10.8,0,Math.PI*2);ctx.fill();for(let dotIndex=0;dotIndex<FRAME_COUNT;dotIndex+=1){const normalized=(dotIndex-frameIndex+FRAME_COUNT)%FRAME_COUNT;const alpha=1-normalized/FRAME_COUNT;const angle=(Math.PI*2*dotIndex)/FRAME_COUNT-Math.PI/2;const x=16+Math.cos(angle)*8.2;const y=16+Math.sin(angle)*8.2;ctx.beginPath();ctx.fillStyle=`rgba(255, 255, 255, ${Math.max(0.2,alpha)})`;ctx.arc(x,y,2.45,0,Math.PI*2);ctx.fill();}ctx.beginPath();ctx.strokeStyle="rgba(255, 255, 255, 0.38)";ctx.lineWidth=1.15;ctx.arc(16,16,11.6,0,Math.PI*2);ctx.stroke();frames.push(canvas.toDataURL("image/png"));}return frames;}
...
function ensureGeneratedIconLink(){let link=document.getElementById(EXT_ICON_LINK_ID);if(link)return link;removeAllIconLinks();link=document.createElement("link");link.id=EXT_ICON_LINK_ID;link.rel="icon";document.head.appendChild(link);return link;}
